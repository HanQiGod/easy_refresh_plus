...
class _SpaceIndicator extends StatefulWidget {
  ...
  const _SpaceIndicator({Key? key, required this.state, required this.reverse})
    : super(key: key);
...
class _SpaceIndicatorState extends State<_SpaceIndicator> {
  ...
  void initState() {
    super.initState();
    _pullController = SimpleAnimation('Pull', autoplay: false);
    ...
    _loadingController = SimpleAnimation('Loading', autoplay: false);
  ...
  void _onModeChange(IndicatorMode mode, double offset) {
    ...
    if (mode == IndicatorMode.drag || mode == IndicatorMode.armed) {
      if (_triggerController.isActive) {
        ...
        _triggerController.instance?.animation.apply(
          0,
          coreContext: _artboard!,
        );
      }
    }
  ...
  Widget build(BuildContext context) {
    if (_artboard != null) {
      if (_mode == IndicatorMode.drag ||
        ...
        _pullController.instance?.animation.apply(
          scale,
          coreContext: _artboard!,
        );
      }
    ...
    return SizedBox(
      ...
      child: RiveAnimation.asset(
        ...
        controllers: [_pullController, _triggerController, _loadingController],
        fit: BoxFit.cover,
        onInit: (artboard) {
